pipeline_cross_validation <- function(datarenum,formula,group_name_cv,LOO_cv,
                                      ...
                                      genotype_file,n_threads,method="blupf90+"){
  orig <- getwd()
  fold_LR <- c()
...
ncol_sol <- ncol(fread(solut,skip = 1,nrows = 2))
  if(ncol_sol<6){
...
full_ebv <- fread(solut,skip = 1,col.names=col_sol) %>% 
...
##################################
    ...
    Time_var <-seq(Timemin,Timemax)
          ...
          fold_sol <- 
            fread(solut,skip = 1,col.names=col_sol) %>% 
