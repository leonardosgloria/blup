...
RRM_cor <- function(log_name,vc_names,parms_card,DAPmin,DAPmax){
  #vc_names <- c("add")
  ###############################
  ...
  for (vc in 1:length(vc_names)) {
    ...
    vc_matrix <- read_lines(log_name)[seq(pos_VC[vc],pos_VC[vc]+length(VC_[[vc]])*number_traits-1)] %>% 
